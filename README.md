...
### 카테고리 관리 페이지 개선 및 공통 컴포넌트 분리

- **카테고리 관리 테이블/페이징 공통 컴포넌트 분리**
  - `src/components/CommonTable.tsx` : 컬럼, 데이터, 로딩, 빈 메시지, 행 렌더링 함수를 prop으로 받아 재사용 가능한 테이블 컴포넌트 생성
  - `src/components/CommonPaging.tsx` : 현재 페이지, 전체 페이지, 페이지 변경 핸들러, 스타일을 prop으로 받아 재사용 가능한 페이징 컴포넌트 생성
  - 카테고리 관리 페이지(`BlogCategoryList.tsx`)에서 위 컴포넌트로 테이블/페이징 UI를 공통화

- **서버 페이징 동기화**
  - 카테고리 목록 조회 시 GET 파라미터로 `page`, `size`, `search`를 서버에 전달하여 서버 페이징/검색과 동기화
  - 서버 응답에서 `categories`(또는 `content`), `totalPages`(또는 `lastPage`)를 받아 프론트 페이징에 반영

- **검색/목록 개수/페이징 기능 추가**
  - 카테고리명/설명 실시간 검색 필터링
  - 한 페이지에 보여줄 개수(10, 20, 50) 선택 기능
  - 페이지네이션(이전/다음, 페이지 번호) 기능 구현

- **다크 테마 UI 고도화**
  - 검색창, 셀렉트, 페이징, 테이블 등 전체를 어두운 계열로 통일
  - placeholder, 버튼, border, 포커스, 그라데이션 등 세부 스타일 개선

- **기타**
  - 기존 mock 데이터/주석 정리
  - 공통 컴포넌트 구조로 확장성 및 유지보수성 향상
